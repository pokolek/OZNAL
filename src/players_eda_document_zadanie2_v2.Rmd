...
# Fit Logistic Regression Model with reduced predictors
...
```
# Fit Logistic Regression Model on the full test data
...
# Fit Logistic Regression Model on the subset
...
When determining the optimal number of neighbors (k) for the k-Nearest Neighbors model, I considered how the accuracy of the model changes as k varies. The plot of model accuracy against k displays a pattern where the model's performance improves significantly with an initial increase in the number of neighbors, which is reflective of the model leveraging more data to make informed decisions.
...
Given this observation, I chose k=35, which is on this plateau. This choice is strategic for a few reasons:
...
plot.roc(test_data$IsDefender, logit_predictions, col = "blue")
lines.roc(test_data$IsDefender, naive_predictions_probs, col = "red")
lines.roc(test_data$IsDefender, knn_prob, col = "green")
lines.roc(test_data$IsDefender, rf_prob, col = "purple")
legend("bottomright", legend = c("Logistic", "Naive Bayes", "k-NN", "Random Forest"),
       col = c("blue", "red", "green", "purple"), lwd = 2)
...
The logistic regression model achieved an accuracy of 89.61%, with a kappa value of 0.7833, indicating substantial agreement beyond chance. The sensitivity and specificity were 90.46% and 88.29%, respectively. This model effectively balanced prediction accuracy across classes, demonstrating its robustness in the dataset context .

### k-Nearest Neighbors (kNN) Evaluation
The kNN model, with optimal hyperparameters, showed slightly lower performance compared to logistic regression, with an accuracy of around 85%. It had a sensitivity of approximately 84% and specificity of about 86%. These results highlight the model's reasonable performance but indicate potential for improvement, particularly in handling noise and irrelevant features .

### Naive Bayes Evaluation
Naive Bayes provided the fastest predictions due to its assumption of independence between predictors. However, this assumption also limited its performance, with an accuracy near 80%. This model had lower sensitivity and specificity compared to the other models, reflecting its challenges in handling feature interdependencies effectively .

### Random Forest Evaluation
The Random Forest model displayed excellent accuracy of approximately 91%, outperforming other models in the dataset. With a kappa value of 0.80, it demonstrated substantial agreement beyond chance. The model achieved a sensitivity of 92% and specificity of 89%, indicating strong capability in correctly identifying both defenders and non-defenders. Its AUC of 0.95 highlighted superior discrimination ability across classification thresholds. While the model benefits from robustness against overfitting and insightful feature importance analysis, it is computationally intensive, which may be a limitation for larger datasets or real-time applications.

## Conclusion
Logistic Regression, k-Nearest Neighbors (kNN), Naive Bayes, and Random Forest—revealed distinct strengths and weaknesses in the context of classifying soccer players as defenders or non-defenders. Logistic Regression emerged as a particularly strong performer with an optimal balance of accuracy, sensitivity, and specificity, making it highly effective for this dataset. Random Forest also demonstrated superior capability, especially in handling complex model interactions and providing high discrimination ability, as evidenced by the highest AUC score among the models.

While kNN and Naive Bayes showed reasonable effectiveness, they were slightly outperformed in terms of overall accuracy and the ability to manage feature interdependencies. Naive Bayes was noted for its speed and efficiency, which could be advantageous in scenarios requiring rapid predictions despite its lower sensitivity and specificity.

Considering the insights from the model evaluations, Logistic Regression is recommended for continued use due to its robustness and reliability. However, where computational resources and time are not limiting factors, Random Forest could be a preferable alternative due to its higher performance metrics and insightful feature importance.